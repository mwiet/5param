...
def load_obs(objname, filterset=["sdss_g0", "sdss_r0"],
             **kwargs):
    obs = {}
...
def pippi_format(filename, MPIrank=0):
    ...
    hf_new['sampling'].create_dataset('multiplicity', data = np.repeat(1, len(ln_flat)))
    hf_new['sampling'].create_dataset('multiplicity_isvalid', data = np.repeat(1, len(ln_flat)))
...
def load_model(zred=None, add_neb=False, complex_atten=False, atten_bump=False, **extras):
    ...
    # For speed we turn off nebular emission
