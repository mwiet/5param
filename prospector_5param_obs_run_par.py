...
# THETA
run_params = {}
sample = pd.read_csv('c15_clean_264.csv')
for i in range(len(sample)):
    index = int(os.environ['PBS_ARRAY_INDEX'])-1
    if i == index:
        run_params["object_redshift"] = sample['ZPDF'][i]
        run_params['objname'] = sample['NUMBER'][i]
...
run_params['massmet'] = True #Include mass-metallicity prior based on Gallazy et al., 2005
...
outroot = "_5bands_{0}_{1}".format(run_params['outfile'], run_params['objname'])
